...
async function initalize() {
  ...
  // get access token
  var result = await fetch(getTokenUrl, {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    if (payload.type == "replace_state") {
      ...
      if (isSimulatingStateChnage) {
        ...
        return new MessageEvent(messageEvent.type, {
          data: "{}",
        });
      }
...
function onAdCouldntBeRemoved(trackURI) {
  ...
  if (now - lastMissedAdTime > 60000) {
    Swal.fire({
      ...
      html:
        "It appears that an ad was missed and couldn't be removed. Please report that back to the developer.",
      icon: "warning",
      width: 600,
...
function checkInterception() {
  ...
  if (isInterceptionWorking) {
    ...
    Swal.fire({
      title: "Oops...",
      html:
        "Spotify Ads Remover has detected that interception is not fully working. Please try refreshing this page, or, if the problem presists, writing back to the developer.",
      icon: "error",
      width: 600,
      confirmButtonColor: "#DD6B55",
...
function showMultiDeviceWarning() {
  if (!didShowMultiDeviceWarning) {
    Swal.fire({
      ...
      html:
        "Please note that Spotify Ads Remover can't control other over playing devices, so ads will not be removed unless audio will play from this tab.",
      icon: "warning",
      width: 500,
      confirmButtonColor: "#DD6B55",
      confirmButtonText: "OK",
